...
public class SysUserDao extends MyBatisDao {
	 ...
	 * @return
	 */
	public SysUserVO getUserByAccount(SysUserVO userVO){
		return (SysUserVO) this.getObject("sysUser.getUserByAccount", userVO);
	}
	 ...
	 * @return
	 */
	public boolean updatePassword(SysUserVO userVO){
		boolean flag = false;
		try {
			this.update("sysUser.updatePassword", userVO);
			flag = true;
		} catch (Exception e) {
			e.printStackTrace();
